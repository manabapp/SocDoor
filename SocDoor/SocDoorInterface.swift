...
struct SocDoorInterface {
    ...
    var name: String { return Self.deviceNames[self.deviceType] }
    ...
    mutating func ifconfig() {
        ...
        while ifaPtr != nil {
            guard let ifa = ifaPtr?.pointee else {
                SocLogger.error("SocDoorInterface.ifconfig: null pointer")
                return
            }
            guard let ifaName = String(validatingUTF8: ifa.ifa_name) else {
                SocLogger.error("SocDoorInterface.ifconfig: no name")
                return
            }
